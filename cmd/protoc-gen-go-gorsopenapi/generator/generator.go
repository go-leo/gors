...
const (
	infoURL = "https://github.com/go-leo/gors/tree/master/cmd/protoc-gen-go-gorsopenapi"
)
...
func NewOpenAPIv3Generator(plugin *protogen.Plugin, conf Configuration, inputFile *protogen.File, service *protogen.Service) *OpenAPIv3Generator {
...
func (g *OpenAPIv3Generator) Run(outputFile *protogen.GeneratedFile) error {
	...
	if err != nil {
		return fmt.Errorf("failed to build document: %s", err.Error())
	}
	bytes, err := d.YAMLValue("Generated with protoc-gen-go-gorsopenapi\n" + infoURL)
	if err != nil {
...
func (g *OpenAPIv3Generator) buildDocumentV3() (*v3.Document, error) {
	...
	serviceInfo, err := parser.ParseServiceFromPb(g.service, *g.conf.PathToLower)
	if err != nil {
		return nil, err
	}
	if err := g.addPathsToDocumentV3(d, g.service, serviceInfo); err != nil {
		return nil, err
	}
...
func (g *OpenAPIv3Generator) addOperationToDocumentV3(d *v3.Document, op *v3.Operation, path string, methodName parser.Method) error {
	selectedPathItem, ok := slicex.FindFunc(d.Paths.Path, func(item *openapi_v3.NamedPathItem) bool { return item.Name == path })
	if !ok {
		selectedPathItem = &v3.NamedPathItem{
			Name:  path,
			Value: &v3.PathItem{},
		}
	...
	case parser.TRACE:
		...
		return nil
	case parser.CONNECT:
		return errors.New("CONNECT method unsupported")
	default:
...
func (g *OpenAPIv3Generator) addPathsToDocumentV3(d *v3.Document, service *protogen.Service, serviceInfo *parser.ServiceInfo) error {
	for _, router := range serviceInfo.Routers {
		method := router.ProtoMethod
		...
		defaultHost := ""
		body := "*"
		joinPath, err := url.JoinPath(serviceInfo.BasePath, router.Path)
		if err != nil {
			return err
		}
		op, path2 := g.buildOperationV3(
			d, operationID, service.GoName, comment, defaultHost, joinPath, body, inputMessage, outputMessage)
		if err := g.addOperationToDocumentV3(d, op, path2, router.HttpMethod); err != nil {
			return err
		}
	}
